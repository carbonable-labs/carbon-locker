...
mod LockerComponent {
    ...
    struct Storage {
        ...
        penalty_multiplier: felt252, // Multiplier for penalty calculations (e.g., 0.05 for 5%)
    ...
    pub struct PenaltyConfigSet {
        pub penalty_multiplier: felt252,
    ...
    mod Errors {
        ...
        const INVALID_WITHDRAW_AMOUNT: felt252 = 'Withdraw amount exceeds locked amount';
    ...
    > of ILockerHandler<ComponentState<TContractState>> {
        ...
        fn lock_credits(
            ...
            let project_felt: felt252 = project_address.into();
            println!("project_felt: {:?}", project_felt);
        ...
        fn terminate_lock_with_penalty(
            ...
            let current_time: u256 = get_block_timestamp().into();
            let remaining_time: u256 = if current_time >= lock.end_time {
                0
            } else {
                lock.end_time - current_time
            };
            let total_lock_time: u256 = lock.end_time - lock.start_time;
            let penalty_amount: u256 = (withdraw_amount * self.penalty_multiplier.read() * remaining_time) / total_lock_time;
        ...
        fn set_penalty_config(
            ref self: ComponentState<TContractState>,
            penalty_multiplier: felt252,
            penalty_recipient: ContractAddress
        ) {
            self.assert_only_role(OWNER_ROLE);
            assert(penalty_multiplier <= 1, Errors::INVALID_PENALTY_MULTIPLIER);
    ...
    > of InternalTrait<TContractState> {
        fn initializer(
            ...
            self.penalty_multiplier.write(0.05); // 5% penalty by default
