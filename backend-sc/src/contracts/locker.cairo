...
#[starknet::contract]
mod Locker {
    use LockerComponent::HasComponent;
    use starknet::ContractAddress;
    ...
    // Embed components in a specific order to avoid method conflicts
    component!(path: OwnableComponent, storage: ownable, event: OwnableEvent);
    component!(path: UpgradeableComponent, storage: upgradeable, event: UpgradeableEvent);
    component!(path: SRC5Component, storage: src5, event: SRC5Event);
    component!(path: LockerComponent, storage: locker_handler, event: LockerEvent); // Embedded before AccessControlComponent
    ...
    fn constructor(
        ...
        self.locker_handler.set_penalty_config(0.05, penalty_recipient); // 5% penalty by default
    }
}
